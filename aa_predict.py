...
(X_train, y_train), in_shape, data_name = wtrain.load_data("data/" + TRAIN_NAME + "." + str(SEQ_LEN) + ".csv.gz", "x", 
                                                           LEFT_WIND, RIGHT_WIND, 
                                                           add_pos=True, for_rnn=True)
(X_test, y_test), _, _ = wtrain.load_data("data/" + TEST_NAME + "." + str(SEQ_LEN) + ".csv.gz", "x", 
                                          LEFT_WIND, RIGHT_WIND, add_pos=True, for_rnn=True)

beholder = wtrain.Beholder(folderpath=FOLDER_NAME, seq_len=SEQ_LEN, 
